...
const stableStringify = require("json-stable-stringify") // 让输出的json顺序不改变
...
async function wirteFun(currentSelect, trsName) {
  ...
  const orderedJsonString = stableStringify(fileObj, { space: 2 })

  fs.writeFile(writeDir, orderedJsonString, (err) => {
    if (err) return
...
async function main(type) {
  ...
  let currentSelect = activeTextEditor.document.getText(
  ...
  //随机转换状态
  const freeTrans = vscode.workspace
    .getConfiguration("zhtrans")
    .get("freeTrans")
  let sendText = ""
  //转换
  if (transWord) {
    ...
    if (freeTrans) {
      transTxt(type, false, currentSelect, (data, trsName) => {
        activeTextEditor.edit((editBuilder) => {
          editBuilder.replace(activeTextEditor.selection, data)
        })
        // 随机转换写入
        const transWrite = vscode.workspace
          .getConfiguration("zhtrans")
          .get("transWrite")
        if (transWrite) {
